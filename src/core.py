...
class GameSourceCode:
    ...
    def __init__(self, client: httpx.AsyncClient, total: bool = False):
        ...
        self._commit: dict[str, Any] | None = None  # type: ignore
    ...
    async def get_latest_commit(self) -> None:
        ...
        logger.info(locale(Langs.GetLatestCommitStartInfo,
                    url=self.REPO_COMMITS_URL))
        response = await self._client.get(self.REPO_COMMITS_URL, params={"ref_name": "master"})
        repo_json = response.json()
        latest_commit = repo_json[0]
        logger.info(locale(Langs.GetLatestCommitMsgInfo,
                    latest_commit=latest_commit["id"]))

        self._is_latest = bool(
            self._commit and latest_commit["id"] == self._commit["id"])
        if self._is_latest:
            return None
        ...
        logger.info(locale(Langs.GetLatestCommitFinishInfo,
                    file=self.FILE_COMMITS))
    ...
    async def download(self):
        ...
        tasks = [
            chunk_download(
                self.REPO_ZIP_URL, self._client, start, end, idx, len(
                    chunks), self.FILE_GAME_ZIP  # type: ignore
            ) for idx, (start, end) in enumerate(chunks)  # type: ignore
        ]
        ...
        for name in os.listdir(DIR_MODS_ROOT):
            ...
            for dir_name in os.listdir(DIR_MODS_ROOT / name):
                ...
                self._boot_json[name]["scriptFileList"] = data.get(
                    "scriptFileList", [])
                self._boot_json[name]["tweeFileList"] = data.get(
                    "tweeFileList", [])
                self._boot_json[name]["imgFileList"] = data.get(
                    "imgFileList", [])
            ...
            if not boot_flag:
                ...
                # 使用 open() 函式以寫入模式創建文件
                with open(DIR_MODS_ROOT / name / "boot.json", 'w') as file:
                    file.write('{}')
                logger.info(locale(Langs.add_bootjson))
        ...
        for name in os.listdir(DIR_MODS_ROOT):
            ...
            for root, dir_list, file_list in os.walk(DIR_MODS_ROOT / name):
                for file in file_list:
                    filepath = "/".join((Path(root) / file).__str__().replace(
                        "\\", "/").split(f"{name}/")[1:])
                    ...
                    if file.endswith(".twee"):
                        ...
                        None if (DIR_SOURCE_REPO / filepath).exists(
                        ) else self._boot_json[name]["tweeFileList"].append(filepath_str)
                        ...
                        if not any({(
                            ...
                            self._boot_json[name]["scriptFileList"].append(
                                filepath_str)

                    elif file.endswith(".css"):
                        self._boot_json[name]["styleFileList"].append(
                            filepath_str)
                    elif any(file.endswith(suf) for suf in {
                        ".jpg", ".png", ".gif", "svg"
                    }):
                        self._boot_json[name]["imgFileList"].append(
                            filepath_str)

                    elif file.endswith(".js.map"):
                        self._boot_json[name]["additionFile"] = self._boot_json[name].get(
                            "additionFile", [])
                        self._boot_json[name]["additionFile"].append(
                            filepath_str)
                    ...
                    if not (DIR_RESULTS_ROOT / name / filepath).parent.exists():
                        os.makedirs((DIR_RESULTS_ROOT / name /
                                    filepath).parent, exist_ok=True)

                    if not file.endswith(".twee"):
                        if not (DIR_TEMP_ROOT / name / filepath).parent.exists():
                            os.makedirs(
                                (DIR_TEMP_ROOT / name / filepath).parent, exist_ok=True)
                        shutil.copyfile(
                            Path(root) / file,
                        ...
                        for filedir in os.listdir(DIR_TEMP_ROOT / name):
                            if (DIR_TEMP_ROOT / name / filedir).is_file():
                                if not (DIR_SOURCE_REPO / filedir).parent.exists():
                                    os.makedirs(
                                        (DIR_SOURCE_REPO / filedir).parent, exist_ok=True)
                                shutil.copyfile(
            ...
            for list_name in {"tweeFileList", "scriptFileList", "styleFileList", "imgFileList"}:
                self._boot_json[name][list_name] = list(
                    set(self._boot_json[name][list_name]))

            with open(DIR_RESULTS_ROOT / name / "boot.json", "w", encoding="utf-8") as fp:
                json.dump(self._boot_json[name], fp,
    ...
    def _process_passage(self, filepath: Path, name: str):
        ...
        if not (DIR_SOURCE_REPO / filepath).exists():
            if not (DIR_TEMP_ROOT / name / filepath).parent.exists():
                os.makedirs(
                    (DIR_TEMP_ROOT / name / filepath).parent, exist_ok=True)
            # logger.info(DIR_MODS_ROOT)
            # logger.info(DIR_TEMP_ROOT)
            # logger.info(name)
            # logger.info(filepath)
            # logger.info(os.path.join(DIR_MODS_ROOT / name / filepath))
            # logger.info(os.path.join(DIR_TEMP_ROOT / name / filepath))
            shutil.copyfile(
                os.path.join(DIR_MODS_ROOT, name, filepath),
                os.path.join(DIR_TEMP_ROOT, name, filepath)
            )
        ...
        for source_idx, source_passage in enumerate(source_passages_info):
            ...
            if source_idx != len(source_passages_info) - 1:
                # source_lines: 原文件行
                slice = source_lines[source_passages_info[source_idx]["start_line"]
                    : source_passages_info[source_idx+1]["start_line"]-1]
                source_lines[source_passages_info[source_idx]["start_line"]
                    : source_passages_info[source_idx+1]["start_line"]-1] = ["" for _ in range(len(slice))]
            else:
                source_lines[source_passages_info[source_idx]
                             ["start_line"]:] = [""]

        source_lines.extend(mod_lines)
        if not (DIR_TEMP_ROOT / name / filepath).parent.exists():
            os.makedirs((DIR_TEMP_ROOT / name / filepath).parent,
                        exist_ok=True)
        ...
        for name in os.listdir(DIR_RESULTS_ROOT):
            ...
            with ZipFile(DIR_RESULTS_ROOT / f"{filename}.mod.zip", "w", compression=ZIP_DEFLATED, compresslevel=5) as zfp:
                for root, dir_list, file_list in os.walk(DIR_RESULTS_ROOT / name):
                    for file in file_list:
                        zfp.write(filename=Path(
                            root) / file, arcname=(Path(root) / file).relative_to(DIR_RESULTS_ROOT / name))
            shutil.copyfile(
